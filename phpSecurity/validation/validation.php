...
class Validation {
    ...
    public function billtrust($value, $fieldname) {
        $string = '@billtrust.com';
        $valid = $string == substr($value, strrpos($value, $string));
        if ($valid == FALSE) $this->errors[] = "The $fieldname is not a valid email";
        return $valid;
    } 
}
?>